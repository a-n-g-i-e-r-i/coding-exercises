package com.angieri.w005;

import com.angieri.Main;
...
*
* Your task is to run Dijkstra's shortest-path algorithm on this graph, using 1 (the first vertex) as the source vertex, and to
* compute the shortest-path distances between 1 and every other vertex of the graph. If there is no path between a vertex v and
* vertex 1, we'll define the shortest-path distance between 1 and v to be 1000000.
*
...
* 115 are at distance 1000 away from vertex 1 and 115 is 2000 distance away, then your answer should be
* 1000,1000,1000,1000,1000,2000,1000,1000,1000,1000. Remember the order of reporting DOES MATTER, and the string should be in the
*  same order in which the above ten vertices are given. The string should not contain any spaces. Please type your answer in the
*  space provided.
*
...
*  OPTIONAL: For those of you seeking an additional challenge, try implementing the heap-based version. Note this requires a
*  heap that supports deletions, and you'll probably need to maintain some kind of mapping between vertices and their positions in the heap.
*/
public class DijkstraShortestPath {
    ...
    private static ArrayList<Vertex> readGraphFromFile()
    {
        ...
        try
        {
            fstream = new FileInputStream("dijkstra-adjacency-list.txt");
            // Vertices:    7,37,59,82,99,115,133,165,188,197
            // Distances:   2599,2610,2947,2052,2367,2399,2029,2442,2505,3068

//            fstream = new FileInputStream("dijkstra-test-1.txt");
